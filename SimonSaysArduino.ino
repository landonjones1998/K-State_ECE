...
void loop()
{
  ...
  if (gameMode == MODE_MEMORY)
  {
    ...
    if (play_memory() == true)
    { 
      play_winner(); // Player won, play winner lights
      lcd.setCursor(0,1);
      lcd.print("Score: " + String(score));
    }
    else 
      play_loser(); // Player lost, play loser lights
      lcd.setCursor(0,1);
      lcd.print("Score: 00");
      delay(2000);
...
void play_loser(void)
{
  ...
  setLEDs(CHOICE_RED | CHOICE_BLUE);
  delay(1000);
  setLEDs(CHOICE_GREEN | CHOICE_RED | CHOICE_YELLOW | CHOICE_BLUE);
}
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
int LEDnumber = 0; // Keeps track of which LED
